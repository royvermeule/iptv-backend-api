...
class SelectProfileService
{
    ...
    public function select(string $userId, string $profileId): array
    {
        ...
        if (!$profile->hasCredentials()) {
            throw new \DomainException('Profile has no IPTV credentials configured', 422);
        }
        ...
        if (!(new XtreamService())->testCredentials($url, $username, $password)) {
            throw new \DomainException('IPTV credentials are invalid or the server is unreachable', 422);
        }
        ...
        $this->redis->setex('refresh_lookup:' . $refreshToken, 604800, $userId . '|' . $profileId);
